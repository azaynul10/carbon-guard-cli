...
try:
    ...
    OCR_AVAILABLE = False
    print(
        "⚠️  OCR libraries not available. Install with: pip install pillow pytesseract"
    )
...
def parse_receipt_text(text: str) -> Dict:
    ...
    for line in lines:
        ...
        if any(
            word in line.lower()
            for word in ["total", "subtotal", "tax", "change", "cash", "card"]
        ):
            # Extract totals
...
def clean_item_name(item_text: str) -> str:
    ...
    item_text = re.sub(
        r"\d+(?:\.\d+)?\s*(lb|kg|oz|g|lbs)\s*", "", item_text, flags=re.IGNORECASE
    )
...
def estimate_co2_emissions(receipt_data: Dict) -> Dict:
    ...
    for item in receipt_data.get("items", []):
        ...
        if emission_factor > 0:
            ...
            category_group = (
                matched_category.split("_")[0]
                if "_" in matched_category
        ...
        "items": [
            {
                ...
                "quantity": 1.0,
                "unit_price": 12.99,
                "raw_line": "Ground Beef 1lb $12.99",
            },
            {
                "name": "chicken breast",
                "price": 8.49,
            ...
            },
            {
                ...
                "quantity": 1.0,
                "unit_price": 2.49,
                "raw_line": "Whole Wheat Bread $2.49",
            },
            {
                "name": "bananas",
                "price": 1.89,
...
def print_receipt_analysis(receipt_data: Dict, co2_data: Dict) -> None:
    ...
    print(
        f"   Total CO2: {co2_data['total_co2_kg']:.3f} kg ({co2_data['total_co2_kg']*1000:.1f}g)"
    )
    ...
    if co2_data["category_breakdown"]:
        print(f"\n📊 CO2 BY CATEGORY:")
        for category, co2_kg in sorted(
            co2_data["category_breakdown"].items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (
                (co2_kg / co2_data["total_co2_kg"]) * 100
                if co2_data["total_co2_kg"] > 0
                else 0
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract items from receipt and estimate CO2 emissions"
    )
    ...
    return {"receipt_data": receipt_data, "co2_analysis": co2_data}
