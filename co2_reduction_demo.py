...
def demo_reduction_planning():
    ...
    aws_plan = planner.generate_reduction_plan(
        target_reduction=30.0, timeframe_months=6, focus_areas=["aws"]
    )
    ...
    personal_plan = planner.generate_reduction_plan(
        target_reduction=25.0, timeframe_months=12, focus_areas=["personal"]
    )
...
def demo_csv_export():
    ...
    exported_files = exporter.export_all_data(
        output_prefix="demo_carbon_export", data_directory="carbon_data"
    )
    ...
    if summary_file:
        ...
        with open(summary_file[0]) as f:
            ...
            for row in reader:
                ...
                print(
                    f"   {category.upper()}: {metric} = {value} {unit} ({points} data points)"
                )
