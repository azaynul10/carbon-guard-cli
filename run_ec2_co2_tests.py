...
def demo_ec2_co2_calculation_with_moto():
    ...
    for instance in result['instances']:
        ...
        print(f"   {state_emoji} {instance['instance_id']} ({instance['instance_type']})")
    ...
    if stopped_count > 0:
        print(f"   🔴 {stopped_count} stopped instance(s) not shown (filtered out by auditor)")
        print()
    ...
    print(f"   Annual CO2 emissions: {result['co2_kg_per_hour'] * 24 * 365:.4f} kg/year")
    ...
    for instance_type, data in power_by_type.items():
        ...
        print(f"   {instance_type}: {data['count']} instances, {avg_power}W each, {data['total_power']}W total")
...
def demo_regional_carbon_intensity():
    ...
    print(f"{'Region':<20} {'Carbon Intensity':<18} {'CO2/hour':<12} {'CO2/year':<12} {'Rank'}")
    print("-" * 80)
    
    for i, result in enumerate(results, 1):
        rank_emoji = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else "📍"
        print(f"{result['region']:<20} {result['carbon_intensity']:<18.6f} "
    ...
    print(f"   Annual CO2 savings: {savings_kg:.4f} kg ({savings_percent:.1f}% reduction)")
