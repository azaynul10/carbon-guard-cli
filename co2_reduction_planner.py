...
class CO2ReductionPlanner:
    ...
    def __init__(self):
        ...
        self.aws_actions = [
            ...
            },
            {
                ...
                'timeline_weeks': 6,
                'applicability': ['ec2'],
                'prerequisites': ['Load balancer setup', 'Application stateless design'],
                'implementation_steps': [
        ...
        for filename in os.listdir(data_directory):
            if filename.endswith('.json'):
                ...
                try:
                    ...
                    if 'service' in data or any(service in data for service in ['ec2', 'rds', 'lambda', 's3']):
                        audit_data['aws'].append(data)
                        audit_data['total_co2_kg'] += data.get('co2_kg_per_hour', 0) * 24 * 30  # Monthly estimate
                        ...
                        if 'summary' in data:
                            audit_data['total_co2_kg'] += data['summary'].get('total_co2_kg', 0)
    ...
    def generate_reduction_plan(self, target_reduction: float = 20.0, 
        ...
        if not audit_data['data_sources']:
            available_actions = self.aws_actions + self.personal_actions + self.dev_actions
        ...
        timeline = self._create_implementation_timeline(selected_actions, timeframe_months)
    ...
    def _select_optimal_actions(self, available_actions: List[Dict], 
                               target_reduction: float, timeframe_months: int) -> List[Dict]:
        ...
        for action in available_actions:
            ...
            timeline_fit = 1.0 if action['timeline_weeks'] <= timeframe_months * 4 else 0.5
            
            # Calculate impact/effort ratio
            impact_effort_ratio = (action['co2_reduction_percent'] / effort_score) * timeline_fit
        ...
        for score, action in scored_actions:
            if cumulative_reduction < target_reduction:
                ...
                additional_reduction = action['co2_reduction_percent'] * (1 - cumulative_reduction / 100)
    ...
    def _create_implementation_timeline(self, actions: List[Dict], 
        ...
        sorted_actions = sorted(actions, key=lambda x: (x['timeline_weeks'], x['effort_level']))
    ...
    def _calculate_plan_metrics(self, actions: List[Dict], audit_data: Dict) -> Dict[str, Any]:
        ...
        avg_complexity = sum(effort_scores[action['effort_level']] for action in actions) / len(actions) if actions else 0
        ...
        estimated_co2_savings = baseline_co2 * (total_reduction / 100) if baseline_co2 > 0 else 0
        ...
        return {
            ...
            'implementation_duration_weeks': sum(action['timeline_weeks'] for action in actions),
            'categories_covered': list(set(action['category'] for action in actions))
        }
    ...
    def _create_monitoring_plan(self, actions: List[Dict]) -> Dict[str, Any]:
        ...
        monitoring_plan = {
            ...
            'monitoring_methods': {},
            'reporting_schedule': [
                {'frequency': 'weekly', 'audience': 'implementation_team', 'format': 'status_update'},
                {'frequency': 'monthly', 'audience': 'management', 'format': 'dashboard'},
                {'frequency': 'quarterly', 'audience': 'stakeholders', 'format': 'detailed_report'}
            ]
        }
        ...
        if 'infrastructure' in categories:
            monitoring_plan['monitoring_methods']['aws'] = 'CloudWatch metrics and cost reports'
        
        if 'diet' in categories:
            monitoring_plan['monitoring_methods']['personal'] = 'Receipt tracking and food diary'
        
        if 'code' in categories:
            monitoring_plan['monitoring_methods']['development'] = 'Performance profiling and energy monitoring'
    ...
    def print_reduction_plan(self, plan: Dict[str, Any]) -> None:
        ...
        for i, action in enumerate(plan['selected_actions'], 1):
            ...
            print(f"   💰 Cost: {'$' + str(abs(action['cost_impact'])) + ' savings' if action['cost_impact'] < 0 else '$' + str(action['cost_impact']) + ' cost'}")
        ...
        if plan['implementation_timeline']['phases']:
            ...
            print(f"   ⏰ Duration: {next_phase['end_week'] - next_phase['start_week']} weeks")
            print(f"   📝 First step: {next_phase['deliverables'][0] if next_phase['deliverables'] else 'Begin implementation'}")
