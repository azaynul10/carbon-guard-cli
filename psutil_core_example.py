...
def monitor_python_script_co2(script_path: str, duration: int = 60):
    ...
    avg_script_cpu = sum(script_cpu_samples) / len(script_cpu_samples) if script_cpu_samples else 0
...
def show_psutil_functions():
    ...
    print(f"   Current process memory: {current_proc.memory_info().rss / (1024**2):.1f} MB")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='psutil CPU monitoring for CO2 estimation')
    ...
    parser.add_argument('--functions', action='store_true', help='Show psutil functions')
