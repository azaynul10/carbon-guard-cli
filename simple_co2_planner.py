...
def generate_co2_reduction_plan(target_reduction: float = 25.0) -> Dict:
    ...
    actions.sort(key=lambda x: x['co2_reduction_percent'] / effort_scores[x['effort']], reverse=True)
    ...
    total_timeline = max(action['timeline_weeks'] for action in selected_actions) if selected_actions else 0
    ...
    plan = {
        ...
        'categories_covered': list(set(action['category'] for action in selected_actions))
    }
...
def export_plan_to_csv(plan: Dict, filename: str = None) -> str:
    ...
    with open(filename, 'w', newline='') as csvfile:
        fieldnames = [
            'plan_id', 'created_at', 'target_reduction_percent', 
            'actual_reduction_percent', 'total_cost_savings_usd',
            'implementation_weeks', 'actions_count', 'categories',
            'action_1', 'action_1_reduction', 'action_1_effort',
            'action_2', 'action_2_reduction', 'action_2_effort',
            'action_3', 'action_3_reduction', 'action_3_effort'
        ]
...
def create_sample_audit_data_csv(filename: str = 'sample_audit_data.csv'):
    ...
    with open(filename, 'w', newline='') as csvfile:
        fieldnames = [
            'audit_type', 'timestamp', 'service_or_category', 
            'co2_value', 'unit', 'cost_or_savings', 'optimization_potential'
        ]
