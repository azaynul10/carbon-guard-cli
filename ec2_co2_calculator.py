...
def get_instance_hours_from_cloudwatch(
    instance_id: str,
    start_time: datetime,
    end_time: datetime,
...
def calculate_co2_emissions(
    instance_type: str, running_hours: float
) -> Dict[str, float]:
    """
...
def get_ec2_co2_report(region: str = "us-east-1", days_back: int = 7) -> Dict[str, Any]:
    ...
    for i, instance in enumerate(instances, 1):
        print(
            f"⚡ Analyzing instance {i}/{len(instances)}: {instance['instance_id']} ({instance['instance_type']})"
        )
        ...
        # Calculate CO2 emissions
        emissions_data = calculate_co2_emissions(
            instance["instance_type"], running_hours
        )
        ...
        print(
            f"  💡 Power: {emissions_data['power_watts']}W | "
...
def print_co2_report(report: Dict[str, Any]) -> None:
    ...
    print(
        f"🌿 Total CO2 Emissions: {summary['total_co2_kg']:.3f} kg ({summary['total_co2_g']:.1f}g)"
    )
    print(
        f"📊 Average CO2 per Instance: {summary['average_co2_per_instance_kg']:.3f} kg"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate CO2 emissions for EC2 instances"
    )
    parser.add_argument(
    ...
    try:
        ...
        print(f"\n✅ Analysis complete!")
        print(
            f"🌿 Total CO2 emissions: {summary['total_co2_kg']:.3f} kg over {args.days} days"
        )
