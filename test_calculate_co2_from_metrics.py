...
def test_calculate_co2_from_metrics():
    ...
    for scenario in scenarios:
        ...
        print(f"      Annual CO2 (if run daily): {result['total_co2_kg'] * 365:.4f} kg/year")
        
        print(f"\n   ⚡ Efficiency Metrics:")
        print(f"      CO2 per CPU%: {result['efficiency_metrics']['co2_per_cpu_percent']:.10f} kg")
        print(f"      CO2 per GB memory: {result['efficiency_metrics']['co2_per_gb_memory']:.8f} kg")
    ...
    if len(results) > 1:
        ...
        sorted_results = sorted(results.items(), key=lambda x: x[1].get('total_co2_kg', 0))
        
        print(f"   CO2 Emissions Ranking (lowest to highest):")
        for i, (name, result) in enumerate(sorted_results, 1):
            if 'error' not in result:
                co2_per_hour = result['total_co2_kg'] / result['duration_hours']
                print(f"      {i}. {name}: {result['total_co2_kg']:.8f} kg ({co2_per_hour:.6f} kg/hour)")
        ...
        if len(sorted_results) >= 2:
            ...
            if most_efficient.get('total_co2_kg', 0) > 0:
                efficiency_ratio = least_efficient.get('total_co2_kg', 0) / most_efficient.get('total_co2_kg', 1)
                print(f"\n   Efficiency Ratio: {efficiency_ratio:.2f}x")
                print(f"   The least efficient script produces {efficiency_ratio:.2f}x more CO2")
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            'summary': {
                ...
                'total_samples_analyzed': sum(r.get('samples_analyzed', 0) for r in results.values())
            }
        }, f, indent=2)
