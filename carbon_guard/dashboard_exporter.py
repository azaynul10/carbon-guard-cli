...
class DashboardExporter:
    ...
    def export_dashboard_data(self, data_directory: str, output_path: str,
                            export_format: str = 'csv', start_date: Optional[str] = None,
        ...
        consolidated_data = self._load_and_consolidate_data(data_directory, start_date, end_date)
        ...
        if export_format == 'csv':
            ...
            if PANDAS_AVAILABLE:
                exported_files.extend(self._export_excel(consolidated_data, output_path))
            else:
    ...
    def _load_and_consolidate_data(self, data_directory: str, start_date: Optional[str] = None,
        ...
        consolidated_data['summary_metrics'] = self._generate_summary_metrics(consolidated_data)
    ...
    def _extract_timestamp(self, data: Dict[str, Any], file_path: str) -> datetime:
        ...
        timestamp_fields = ['audit_timestamp', 'created_at', 'timestamp', 'parsing_timestamp']
    ...
    def _should_include_file(self, file_timestamp: datetime, start_dt: Optional[datetime],
                           end_dt: Optional[datetime]) -> bool:
    ...
    def _categorize_and_add_data(self, data: Dict[str, Any], file_path: str,
        ...
        data_with_metadata['file_timestamp'] = self._extract_timestamp(data, file_path).isoformat()
        ...
        elif 'aws' in filename or any(service in data for service in ['ec2', 'rds', 'lambda', 's3']):
            consolidated_data['aws_audits'].append(data_with_metadata)
    ...
    def _generate_summary_metrics(self, consolidated_data: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
        ...
        if local_data:
            ...
            avg_execution_time = sum(item.get('execution_duration_seconds', 0) for item in local_data) / len(local_data)
        ...
        if personal_data:
            ...
            for item in personal_data:
                if 'summary' in item:
                    ...
                    total_personal_co2 += item['carbon_footprint'].get('total_co2_kg', 0)
    ...
    def _export_csv(self, consolidated_data: Dict[str, List[Dict[str, Any]]], output_path: str) -> List[str]:
        ...
        if consolidated_data['personal_audits']:
            ...
            self._export_personal_csv(consolidated_data['personal_audits'], personal_file)
    ...
    def _export_aws_csv(self, aws_data: List[Dict[str, Any]], output_file: str):
        ...
        with open(output_file, 'w', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'service', 'region', 'total_instances', 'co2_kg_per_hour',
                'estimated_cost_usd', 'source_file'
            ]
    ...
    def _export_local_csv(self, local_data: List[Dict[str, Any]], output_file: str):
        ...
        with open(output_file, 'w', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'script_path', 'execution_duration_seconds', 'total_co2_kg',
                'total_energy_kwh', 'avg_cpu_percent', 'peak_memory_mb', 'source_file'
            ]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            ...
            for item in local_data:
                row = {
                    ...
                    'execution_duration_seconds': item.get('execution_duration_seconds', 0),
    ...
    def _export_personal_csv(self, personal_data: List[Dict[str, Any]], output_file: str):
        """Export personal audit data to CSV."""
        with open(output_file, 'w', newline='') as csvfile:
            fieldnames = [
                'timestamp', 'total_receipts', 'total_co2_kg', 'food_co2_kg',
                'transport_co2_kg', 'goods_co2_kg', 'source_file'
            ]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
    ...
    def _export_excel(self, consolidated_data: Dict[str, List[Dict[str, Any]]], output_path: str) -> List[str]:
    ...
    def _export_json(self, consolidated_data: Dict[str, List[Dict[str, Any]]], output_path: str) -> List[str]:
    ...
    def get_summary_statistics(self, data_directory: str, start_date: Optional[str] = None,
        ...
        aws_co2_hourly = sum(item.get('co2_kg_per_hour', 0) for item in consolidated_data['aws_audits'])
        total_co2_kg += aws_co2_hourly * 24  # Daily estimate
        
        # Local CO2
        total_co2_kg += sum(item.get('total_co2_kg', 0) for item in consolidated_data['local_audits'])
        ...
        # Get date range
        all_timestamps = []
        for category_data in [consolidated_data['aws_audits'], consolidated_data['local_audits'], consolidated_data['personal_audits']]:
        ...
        return {
            ...
            'avg_daily_co2_kg': total_co2_kg / max(1, len(set(ts[:10] for ts in all_timestamps))) if all_timestamps else 0
        }
