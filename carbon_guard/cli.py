...
def setup_mock_aws_resources(region: str = 'us-east-1'):
    ...
    if not MOTO_AVAILABLE:
        raise click.ClickException("moto library is required for mock mode. Install with: pip install moto")
    ...
    s3.put_object(Bucket='mock-bucket-small', Key='small-file.txt', Body=b'Small test content')
    s3.put_object(Bucket='mock-bucket-large', Key='large-file.txt', Body=b'Large test content' * 1000)
...
@click.pass_context
def audit_aws(ctx, region: str, services: tuple, profile: Optional[str], 
              output: Optional[str], estimate_only: bool, mock: bool):
    """Estimate AWS infrastructure CO2 emissions via boto3."""
    
    if mock:
        if not MOTO_AVAILABLE:
            click.echo("❌ Mock mode requires moto library. Install with: pip install moto", err=True)
            raise click.Abort()
    ...
    if output:
        ...
        if mock:
            click.echo("🎭 Note: Results are from mock AWS resources, not real infrastructure")
...
@click.option('--include-network', is_flag=True,
              help='Include network usage in calculations')
@click.pass_context
def audit_local(ctx, script_path: str, duration: int, output: Optional[str],
...
@click.option('--strategy', '-s', 
              ...
              default='all', help='Optimization strategy')
@click.option('--dry-run', is_flag=True,
              help='Show optimizations without applying them')
@click.pass_context
def optimize(ctx, dockerfile_path: str, output: Optional[str], 
    ...
    try:
        ...
        if dry_run:
            ...
            click.echo(f"📉 Estimated CO2 reduction: {optimizer.estimate_savings(optimizations):.2f}%")
...
def track_personal(ctx, receipt_images: tuple, output: Optional[str], category: str):
    ...
    try:
        ...
        # Calculate totals
        total_co2 = sum(r['carbon_footprint'].get('total_co2_kg', 0) for r in all_results)
...
@main.command()
@click.option('--target-reduction', '-t', type=float, default=20.0,
              help='Target CO2 reduction percentage (default: 20%)')
@click.option('--timeframe', '-f', type=int, default=12,
              help='Timeframe in months (default: 12)')
@click.option('--focus', type=click.Choice(['aws', 'local', 'personal', 'all']),
              default='all', help='Focus area for optimization')
@click.option('--output', '-o', type=click.Path(),
              help='Output file for the plan (JSON format)')
@click.pass_context
def plan(ctx, target_reduction: float, timeframe: int, focus: str, 
         output: Optional[str]):
    ...
    try:
        ...
        click.echo(f"  • Estimated total reduction: {plan_data['estimated_reduction']:.1f}%")
        ...
        for i, action in enumerate(plan_data['actions'][:5], 1):
            ...
            click.echo(f"     Impact: {action['co2_reduction']:.1f}% | "
...
@click.pass_context
def dashboard(ctx, data_dir: Optional[str], output: str, format: str, 
    ...
    try:
        ...
        if date_range:
            try:
                ...
                click.echo("❌ Invalid date range format. Use YYYY-MM-DD:YYYY-MM-DD", err=True)
                raise click.Abort()
        ...
        if stats:
            ...
            click.echo(f"  • Average daily CO2: {stats.get('avg_daily_co2_kg', 0):.4f} kg")
