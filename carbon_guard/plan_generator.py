...
class PlanGenerator:
    ...
    ACTION_TEMPLATES = {
        'aws': [
            ...
            },
            {
                ...
                'effort_level': 'high',
                'timeline_weeks': 4,
                'prerequisites': ['Load balancer configured', 'Application supports horizontal scaling'],
                'steps': [
            ...
            },
            {
                ...
                'effort_level': 'medium',
                'timeline_weeks': 2,
                'prerequisites': ['Workloads identified', 'Scheduling system available'],
                'steps': [
    ...
    def generate_plan(self, target_reduction: float = 20.0, timeframe_months: int = 12,
        ...
        """
        logger.info(f"Generating CO2 reduction plan: {target_reduction}% in {timeframe_months} months")
        ...
        selected_actions = self._select_actions(available_actions, target_reduction, timeframe_months)
    ...
    def _load_baseline_data(self) -> Dict[str, Any]:
        ...
        try:
                    ...
                    if filename.endswith('.json'):
                        ...
                        try:
                            ...
                            if 'aws' in filename.lower():
                                baseline['aws_co2_kg_per_hour'] += data.get('co2_kg_per_hour', 0)
                                baseline['data_sources'].append(f'AWS audit: {filename}')
                            elif 'local' in filename.lower():
                                baseline['local_co2_kg_per_execution'] += data.get('total_co2_kg', 0)
                                baseline['data_sources'].append(f'Local audit: {filename}')
                            elif 'personal' in filename.lower():
                                baseline['personal_co2_kg_per_month'] += data.get('total_co2_kg', 0)
                                baseline['data_sources'].append(f'Personal audit: {filename}')
            ...
            baseline['total_monthly_co2_kg'] = (
                baseline['aws_co2_kg_per_hour'] * 24 * 30 +  # AWS running 24/7
                baseline['local_co2_kg_per_execution'] * 100 +  # Assume 100 executions/month
                baseline['personal_co2_kg_per_month']
            )
    ...
    def _get_available_actions(self, focus_area: str, baseline_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for action in available_actions:
            ...
            if self._action_applicable(action, baseline_data):
                ...
                adjusted_action = self._adjust_action_for_baseline(action, baseline_data)
                filtered_actions.append(adjusted_action)
        
        return filtered_actions
    
    def _action_applicable(self, action: Dict[str, Any], baseline_data: Dict[str, Any]) -> bool:
    ...
    def _adjust_action_for_baseline(self, action: Dict[str, Any], baseline_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _select_actions(self, available_actions: List[Dict[str, Any]], 
                       target_reduction: float, timeframe_months: int) -> List[Dict[str, Any]]:
        """Select actions to meet the target reduction within timeframe."""
        # Sort actions by impact/effort ratio
        scored_actions = []
        for action in available_actions:
            effort_score = {'low': 1, 'medium': 2, 'high': 3}.get(action['effort_level'], 2)
            ...
            if timeline_score <= timeframe_months:
                impact_effort_ratio = action['co2_reduction'] / (effort_score * timeline_score)
    ...
    def _create_timeline(self, actions: List[Dict[str, Any]], timeframe_months: int) -> Dict[str, Any]:
    ...
    def _calculate_plan_metrics(self, actions: List[Dict[str, Any]], 
        ...
        avg_complexity = sum(effort_scores.get(action['effort_level'], 2) for action in actions) / len(actions) if actions else 0
        
        # Estimate success probability based on complexity and timeline
        complexity_factor = max(0.3, 1.0 - (avg_complexity - 1) * 0.2)
        timeline_factor = max(0.5, 1.0 - len(actions) * 0.05)  # More actions = lower probability
    ...
    def _create_milestones(self, actions: List[Dict[str, Any]], timeframe_months: int) -> List[Dict[str, Any]]:
        ...
        for quarter in range(1, min(5, timeframe_months // 3 + 1)):
            ...
            actions_by_week = [a for a in actions if a['timeline_weeks'] <= milestone_week]
    ...
    def update_plan_progress(self, plan: Dict[str, Any], completed_actions: List[str],
                           actual_reductions: Dict[str, float]) -> Dict[str, Any]:
        ...
        updated_plan['progress'] = {
            ...
            'completion_rate': len(completed_actions) / len(plan['actions']) if plan['actions'] else 0,
            'actual_reductions': actual_reductions,
            'total_actual_reduction': sum(actual_reductions.values()),
            'variance_from_plan': sum(actual_reductions.values()) - plan['estimated_reduction']
        }
