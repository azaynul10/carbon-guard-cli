...
class ReceiptParser:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        if not OCR_AVAILABLE:
            logger.warning(
                "OCR functionality not available. Install Pillow and pytesseract."
            )
    ...
    def _clean_item_name(self, item_text: str) -> str:
        ...
        item_text = re.sub(
            r"\d+(?:\.\d+)?\s*[x@]?\s*", "", item_text, flags=re.IGNORECASE
        )
        item_text = re.sub(
            r"\d+(?:\.\d+)?\s*(lb|kg|oz|g)\s*", "", item_text, flags=re.IGNORECASE
        )
    ...
    def calculate_carbon_footprint(
        self, receipt_data: Dict[str, Any], category_filter: str = "all"
    ) -> Dict[str, Any]:
        ...
        for item in receipt_data.get("items", []):
            ...
            if category:
                ...
                if emission_factor > 0:
                    estimated_amount = self._estimate_amount(
                        item_name, price, quantity
                    )  # Pass item_name, price, quantity
        ...
        carbon_data["category_breakdown"] = dict(
            carbon_data["category_breakdown"]
    ...
    def _estimate_amount(self, item: str, price: float, quantity: float = 1.0) -> float:
        ...
        category = self._categorize_item(
            item
        )  # Assume this method exists; if not, add simple logic here
    ...
    def parse_multiple_receipts(
        self, image_paths: List[str], category_filter: str = "all"
    ) -> Dict[str, Any]:
        ...
        for image_path in image_paths:
            try:
                ...
                carbon_data = self.calculate_carbon_footprint(
                    receipt_data, category_filter
                )
