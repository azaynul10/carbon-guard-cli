...
class LocalAuditor:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.carbon_intensity = self.config.get(
            "carbon_intensity", self.DEFAULT_CARBON_INTENSITY
        )
        self.cpu_tdp = self.config.get("cpu_tdp_watts", self.DEFAULT_CPU_TDP)
        self.memory_power_per_gb = self.config.get(
            "memory_power_per_gb", self.DEFAULT_MEMORY_POWER_PER_GB
        )
    ...
    def _calculate_co2_from_metrics(
        self, monitoring_data, duration, carbon_intensity, cpu_tdp
    ):
        if not monitoring_data:
            return {
                "total_co2_kg": 0.0,
                "power_breakdown": {"cpu": 0, "memory": 0, "disk": 0, "network": 0},
            }
        ...
        total_disk_gb = sum(
            (d["disk_read_bytes"] + d["disk_write_bytes"]) / (1024**3)
            for d in monitoring_data
        )
        total_network_gb = sum(
            (d["network_bytes_sent"] + d["network_bytes_recv"]) / (1024**3)
            for d in monitoring_data
        )
    ...
    def audit_script(
        self, script_path: str, duration: int = 60, include_network: bool = False
    ) -> Dict[str, Any]:
        ...
        monitor_thread = threading.Thread(
            target=self._monitor_resources, args=(duration, include_network)
        )
    ...
    def _calculate_emissions(
        ...
        if self.monitoring_data:
            avg_cpu_percent = sum(d["cpu_percent"] for d in self.monitoring_data) / len(
                self.monitoring_data
            )
            peak_memory_gb = max(d["memory_used_gb"] for d in self.monitoring_data)
            avg_memory_gb = sum(
                d["memory_used_gb"] for d in self.monitoring_data
            ) / len(self.monitoring_data)
        else:
            # Fallback to simple calculation
            avg_cpu_percent = (
                initial_stats["cpu_percent"] + final_stats["cpu_percent"]
            ) / 2
            peak_memory_gb = max(
                initial_stats["memory_used_gb"], final_stats["memory_used_gb"]
            )
            avg_memory_gb = (
                initial_stats["memory_used_gb"] + final_stats["memory_used_gb"]
            ) / 2
        ...
        disk_read_diff = (
            final_stats["disk_io_read_bytes"] - initial_stats["disk_io_read_bytes"]
        )
        disk_write_diff = (
            final_stats["disk_io_write_bytes"] - initial_stats["disk_io_write_bytes"]
        )
        total_disk_io_gb = (disk_read_diff + disk_write_diff) / (1024**3)
        disk_power_watts = min(
            total_disk_io_gb * 2, self.DEFAULT_DISK_POWER
        )  # Cap at default
        ...
        if include_network:
            network_sent_diff = (
                final_stats["network_bytes_sent"] - initial_stats["network_bytes_sent"]
            )
            network_recv_diff = (
                final_stats["network_bytes_recv"] - initial_stats["network_bytes_recv"]
            )
    ...
    def audit_multiple_scripts(
        self,
        script_paths: List[str],
        duration_per_script: int = 60,
        ...
        for script_path in script_paths:
            try:
                script_result = self.audit_script(
                    script_path, duration_per_script, include_network
                )
                results[script_path] = script_result
                total_co2 += script_result["total_co2_kg"]
            except Exception as e:
                logger.error(f"Failed to audit script {script_path}: {e}")
                results[script_path] = {"error": str(e), "total_co2_kg": 0}
    ...
    def get_system_baseline(self, duration: int = 60) -> Dict[str, Any]:
        ...
        # Start monitoring
        monitor_thread = threading.Thread(
            target=self._monitor_resources, args=(duration, True)
        )
        monitor_thread.start()
        ...
        avg_cpu = sum(d["cpu_percent"] for d in self.monitoring_data) / len(
            self.monitoring_data
        )
        avg_memory_gb = sum(d["memory_used_gb"] for d in self.monitoring_data) / len(
            self.monitoring_data
        )
        ...
        baseline_total_power = (
            baseline_cpu_power + baseline_memory_power + 20
    ...
    def _calculate_co2_from_metrics(
        ...
        try:
            ...
            system_cpu_values = [
                d.get("system_cpu_percent", 0) for d in monitoring_data
            ]
            script_cpu_values = [
                d.get("script_cpu_percent", 0) for d in monitoring_data
            ]

            avg_system_cpu_percent = sum(system_cpu_values) / len(system_cpu_values)
            avg_script_cpu_percent = (
                sum(script_cpu_values) / len(script_cpu_values)
                if any(script_cpu_values)
                else 0
            )

            # Calculate memory usage statistics
            memory_gb_values = [d.get("memory_used_gb", 0) for d in monitoring_data]
            script_memory_mb_values = [
                d.get("script_memory_mb", 0) for d in monitoring_data
            ]

            avg_memory_gb = sum(memory_gb_values) / len(memory_gb_values)
            peak_memory_gb = max(memory_gb_values)
            avg_script_memory_mb = (
                sum(script_memory_mb_values) / len(script_memory_mb_values)
                if any(script_memory_mb_values)
                else 0
            )
            ...
            if len(monitoring_data) > 1:
                ...
                disk_read_diff = last_sample.get(
                    "disk_read_bytes", 0
                ) - first_sample.get("disk_read_bytes", 0)
                disk_write_diff = last_sample.get(
                    "disk_write_bytes", 0
                ) - first_sample.get("disk_write_bytes", 0)
                ...
                network_sent_diff = last_sample.get(
                    "network_bytes_sent", 0
                ) - first_sample.get("network_bytes_sent", 0)
                network_recv_diff = last_sample.get(
                    "network_bytes_recv", 0
                ) - first_sample.get("network_bytes_recv", 0)
            ...
            co2_per_cpu_percent = (
                total_co2_kg / avg_system_cpu_percent
