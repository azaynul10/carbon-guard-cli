...
def monitor_script_co2(
    script_path: str, duration: int = 60, sample_interval: float = 1.0
) -> Dict[str, Any]:
    """
...
def calculate_co2_from_metrics(
    monitoring_data: List[Dict], duration: float, carbon_intensity: float, cpu_tdp: int
) -> Dict[str, Any]:
    """
    ...
    avg_system_cpu = sum(d["system_cpu_percent"] for d in monitoring_data) / len(
        monitoring_data
    )
    avg_script_cpu = sum(d["script_cpu_percent"] for d in monitoring_data) / len(
        monitoring_data
    )
    peak_memory_gb = max(d["memory_used_gb"] for d in monitoring_data)
    avg_memory_gb = sum(d["memory_used_gb"] for d in monitoring_data) / len(
        monitoring_data
    )

    # Calculate script-specific metrics
    peak_script_memory_mb = max(d["script_memory_mb"] for d in monitoring_data)
    avg_script_memory_mb = sum(d["script_memory_mb"] for d in monitoring_data) / len(
        monitoring_data
    )
    ...
    if len(monitoring_data) > 1:
        disk_read_diff = (
            monitoring_data[-1]["disk_read_bytes"]
            - monitoring_data[0]["disk_read_bytes"]
        )
        disk_write_diff = (
            monitoring_data[-1]["disk_write_bytes"]
            - monitoring_data[0]["disk_write_bytes"]
        )
        network_sent_diff = (
            monitoring_data[-1]["network_sent_bytes"]
            - monitoring_data[0]["network_sent_bytes"]
        )
        network_recv_diff = (
            monitoring_data[-1]["network_recv_bytes"]
            - monitoring_data[0]["network_recv_bytes"]
        )
    else:
    ...
    total_power_watts = (
        cpu_power_watts + memory_power_watts + disk_power_watts + network_power_watts
    )
...
def print_co2_results(results: Dict[str, Any]) -> None:
    ...
    print(
        f"   ⏱️  Duration: {results.get('execution_duration_seconds', results.get('monitoring_duration_seconds', 0)):.2f} seconds"
    )
    print(f"   ✅ Success: {results.get('execution_successful', 'N/A')}")
    print(
        f"   📈 Samples: {results.get('monitoring_samples', results.get('samples_collected', 0))}"
    )
    ...
    print(f"\n💻 RESOURCE USAGE:")
    print(
        f"   🖥️  Avg CPU: {results.get('avg_system_cpu_percent', results.get('avg_cpu_percent', 0)):.1f}%"
    )
