...
def monitor_script_cpu_co2(script_path: str, duration: int = 60) -> Dict:
    ...
    try:
        ...
        while process.poll() is None and (time.time() - start_time) < duration:
            ...
            print(f"📊 CPU: {cpu_percent:5.1f}% (script: {script_cpu:5.1f}%) | "
    ...
    avg_memory_gb = sum(s['system_memory_gb'] for s in memory_samples) / len(memory_samples)
    ...
    script_power_watts = script_cpu_power_watts + (peak_script_memory_mb / 1024) * MEMORY_POWER_PER_GB
...
def monitor_current_process(duration: int = 30) -> Dict:
    ...
    while (time.time() - start_time) < duration:
        try:
            ...
            print(f"📊 Process CPU: {proc_cpu:5.1f}% | Memory: {proc_memory_mb:6.1f}MB | System CPU: {system_cpu:5.1f}%")
...
def print_results(results: Dict) -> None:
    ...
    # Basic info
    duration = results.get('execution_duration_seconds', results.get('monitoring_duration_seconds', 0))
    ...
    if 'avg_script_cpu_percent' in results:
        ...
        print(f"🖥️  Process CPU: {results['avg_cpu_percent']:.1f}% (avg), {results.get('peak_cpu_percent', 0):.1f}% (peak)")
    
    # Memory metrics
    if 'peak_script_memory_mb' in results:
        print(f"🧠 Script Memory: {results['peak_script_memory_mb']:.1f} MB (peak)")
    else:
        print(f"🧠 Process Memory: {results['avg_memory_mb']:.1f} MB (avg), {results.get('peak_memory_mb', 0):.1f} MB (peak)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Monitor Python script CO2 with psutil')
