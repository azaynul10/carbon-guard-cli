...
def demo_ec2_co2_calculation():
    ...
    sample_instances = [
        {'id': 'i-1234567890abcdef0', 'type': 't3.medium', 'hours': 168},  # 7 days * 24 hours
        {'id': 'i-0987654321fedcba0', 'type': 'm5.large', 'hours': 120},   # Partial uptime
        {'id': 'i-abcdef1234567890', 'type': 'c5.xlarge', 'hours': 84},    # 3.5 days
        {'id': 'i-fedcba0987654321', 'type': 'r5.2xlarge', 'hours': 168},  # Full week
        {'id': 'i-1111222233334444', 'type': 't2.micro', 'hours': 168},    # Always-on small instance
    ]
    ...
    print("-" * 80)
    print(f"{'Instance ID':<20} {'Type':<12} {'Hours':<8} {'Power':<8} {'Energy':<10} {'CO2 (g)':<10}")
    print("-" * 80)
    ...
    for instance in sample_instances:
        ...
        # Print row
        print(f"{instance_id:<20} {instance_type:<12} {running_hours:<8.1f} "
              f"{power_watts:<8}W {energy_kwh:<10.2f} {co2_g:<10.1f}")
    
    print("-" * 80)
    print(f"{'TOTALS':<20} {'':<12} {sum(i['hours'] for i in sample_instances):<8.1f} "
    ...
    print(f"   🌿 Total CO2 Emissions: {total_co2_kg:.3f} kg ({total_co2_kg * 1000:.1f}g)")
    ...
    print(f"   💰 Estimated Monthly CO2: {total_co2_kg * 30 / DAYS_ANALYZED:.3f} kg/month")
    ...
    print(f"   🎯 Highest emitter: {highest_emitter['instance_id']} ({highest_emitter['instance_type']})")
    print(f"      - Emits {highest_emitter['co2_g']:.1f}g CO2 ({highest_emitter['co2_kg']/total_co2_kg*100:.1f}% of total)")
...
if __name__ == "__main__":
    ...
    print(f"\n✅ Demo complete!")
    print(f"🌿 Key takeaway: {results['total_co2_kg']:.3f} kg CO2 from {results['total_instances']} instances over {results['days_analyzed']} days")
